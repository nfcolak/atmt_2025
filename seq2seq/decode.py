...
def beam_search_decode_rel_pruning(model: Seq2SeqModel, src_tokens: torch.Tensor, src_pad_mask: torch.Tensor, max_out_len: int,
                       tgt_tokenizer: spm.SentencePieceProcessor, args, device: torch.device, beam_size: int = 5, alpha: float = 0.7, threshold: float = 0.6):
    """Beam Search decoding compatible with Transformer-based Seq2Seq models. Uses relative threshold pruning."""
    model.eval()
    BOS, EOS, PAD = tgt_tokenizer.bos_id(), tgt_tokenizer.eos_id(), tgt_tokenizer.pad_id()
    ...
    for _ in range(max_out_len):
        ...
        beams = [hyp for hyp in beams if hyp[1] > threshold * max_score]
...
def beam_search_decode_relloc_pruning(model: Seq2SeqModel, src_tokens: torch.Tensor, src_pad_mask: torch.Tensor, max_out_len: int,
                       tgt_tokenizer: spm.SentencePieceProcessor, args, device: torch.device, beam_size: int = 5, alpha: float = 0.7, threshold: float = 0.02):
    """Beam Search decoding compatible with Transformer-based Seq2Seq models. Uses relative local threshold pruning."""
    model.eval()
    BOS, EOS, PAD = tgt_tokenizer.bos_id(), tgt_tokenizer.eos_id(), tgt_tokenizer.pad_id()
    ...
    # __QUESTION 1: what does this line set up and why is the beam represented this way?
    beams = [(torch.tensor([[BOS]], device=device), 0.0, 0.0)] # element 2 is 'score_w' i.e. the score of the last generated word
    for _ in range(max_out_len):
        new_beams = []
        for seq, score in beams:
            if seq[0, -1].item() == EOS:
        ...
        beams = [hyp for hyp in beams if hyp[2] > threshold * max_score_w]
